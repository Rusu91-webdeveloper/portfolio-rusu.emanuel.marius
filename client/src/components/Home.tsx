import React from "react";
import { TypeAnimation } from "react-type-animation";
import { motion } from "framer-motion";
import Globe from "./Globe";
import Skills from "./Skills";

const Home: React.FC = () => {
  const handleDownloadResume = () => {
    // Replace with your actual resume file path
    const resumeUrl = "/resume/Emanuel_Rusu_Resume.pdf";
    window.open(resumeUrl, "_blank");
  };

  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black">
      {/* Hero Section */}
      <div className="container mx-auto px-4 pt-20 pb-32">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          {/* Text Content */}
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5 }}
            className="flex-1">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-4">
              Emanuel Rusu
            </h1>
            <div className="text-xl md:text-2xl text-blue-400 mb-6">
              <TypeAnimation
                sequence={[
                  "Full-Stack Web Developer",
                  2000,
                  "JavaScript Expert",
                  2000,
                  "AI Enthusiast",
                  2000,
                  "Problem Solver",
                  2000,
                ]}
                repeat={Infinity}
              />
            </div>
            <p className="text-gray-300 text-lg mb-8 max-w-2xl">
              Full-Stack Web Developer with expertise in JavaScript, TypeScript,
              Python, AI, and Cloud Computing. Proficient in building scalable
              web applications and AI-powered solutions with the MERN stack.
              Passionate about creating secure, high-performance applications
              with modern technologies.
            </p>
            <div className="flex gap-4 mb-8">
              <motion.a
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                href="mailto:rusu.emanuel.webdeveloper@gmail.com"
                className="bg-blue-500 text-white px-6 py-3 rounded-full font-semibold hover:bg-blue-600 transition-colors">
                Contact Me
              </motion.a>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={handleDownloadResume}
                className="bg-transparent border-2 border-blue-500 text-blue-500 px-6 py-3 rounded-full font-semibold hover:bg-blue-500 hover:text-white transition-all">
                Download Resume
              </motion.button>
            </div>
            <div className="flex gap-4">
              <motion.a
                whileHover={{ scale: 1.1 }}
                href="https://linkedin.com/in/rusu-emanuel-marius"
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-blue-500 transition-colors">
                <svg
                  className="w-6 h-6"
                  fill="currentColor"
                  viewBox="0 0 24 24">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                </svg>
              </motion.a>
              <motion.a
                whileHover={{ scale: 1.1 }}
                href="https://github.com/Rusu91-webdeveloper"
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-white transition-colors">
                <svg
                  className="w-6 h-6"
                  fill="currentColor"
                  viewBox="0 0 24 24">
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                </svg>
              </motion.a>
            </div>
          </motion.div>

          {/* Profile Image */}
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.5 }}
            className="w-64 h-64 relative">
            <div className="w-full h-full rounded-full overflow-hidden border-4 border-blue-500 shadow-xl">
              <img
                src="/images/profile.jpg"
                alt="Emanuel Rusu"
                className="w-full h-full object-cover"
              />
            </div>
            <div className="absolute -bottom-4 -right-4 bg-blue-500 text-white px-4 py-2 rounded-full text-sm font-semibold">
              Available for Work
            </div>
          </motion.div>
        </div>
      </div>

      {/* Skills Section */}
      <Skills />

      {/* Globe Section */}
      <div className="h-screen relative">
        <Globe />
      </div>
    </div>
  );
};

export default Home;
